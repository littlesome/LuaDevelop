...
void DebugFrontend::SetEventHandler(IEvtHandler* eventHandler)
{
...
void DebugFrontend::EventThreadProc()
{
    ...
    while (m_eventChannel.ReadUInt32(eventId))
    {
        ...
        DebugEvent event(static_cast<EventId>(eventId), vm);
    ...
    if (m_eventHandler != NULL)
    {
        DebugEvent event(static_cast<EventId>(EventId_SessionEnd), 0);
        m_eventHandler->AddPendingEvent(event);        
    }
...
std::string DebugFrontend::Evaluate(unsigned int vm, const char* expression, unsigned int stackLevel)
{
	std::string result;
    if (vm == 0)
    {
...
{
    DebugEvent event(EventId_Message, 0);
    event.SetMessage(message);
