...
namespace LuaDebugger
{
    class StackframeUI : DockPanelControl
    {
        ...
        public void AddFrames(StackFrameInfo[] frames)
        {
            lv.Items.Clear();

            foreach (StackFrameInfo item in frames)
            {
                String title = item.function;

                if (item.available)
                    title += "@" + item.file + " line " + (item.line + 1).ToString();

                ListViewItem i = lv.Items.Add(new ListViewItem(new string[] {"", title}, -1));
