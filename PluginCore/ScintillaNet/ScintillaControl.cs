...
namespace ScintillaNet
{
	public class ScintillaControl : Control
	{
		...
		{
            get { return Encoding.GetEncoding(this.CodePage); }
        }
		...
		public int MBSafePosition(int position)
		{
            ...
            else
			{
				...
				int mbpos = this.Encoding.GetByteCount(count);
				return mbpos;
			}
		}
		...
		public int MBSafeCharPosition(int bytePosition)
		{
            ...
            else
			{
				...
				int chrpos = this.Encoding.GetCharCount(bytes, 0, bytePosition);
				return chrpos;
			}
		}
		...
		{
			int mblenght = this.Encoding.GetByteCount(text);
			return mblenght;
        }
        ...
        {
            byte[] raw = this.Encoding.GetBytes(txt);
            return this.Encoding.GetString(raw, 0, Math.Min(raw.Length, bytes)).Length;
        }
