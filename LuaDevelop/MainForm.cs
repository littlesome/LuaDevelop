...
using LuaDevelop.Controls;
using LuaDevelop.Docking;
using LuaDevelop.Utilities;
using LuaDevelop.Managers;
...
namespace LuaDevelop
{
    public class MainForm : Form, IMainForm, IMessageFilter
    {
        ...
        /// <summary>
        /// Is LuaDevelop in multi instance mode?
        /// </summary>
        ...
        /// <summary>
        /// Is LuaDevelop in standalone mode?
        /// </summary>
        ...
        /// <summary>
        /// Does LuaDevelop hold modified documents?
        /// </summary> 
        ...
        public DockContent OpenEditableDocument(String org, Encoding encoding, Boolean restorePosition)
        {
            ...
            else if (file.EndsWith(".delete.ldz"))
            {
                this.CallCommand("RemoveZip", file);
                return null;
            }
            else if (file.EndsWith(".ldz"))
            {
        ...
        {
            Icon icon = new Icon(ResourceHelper.GetStream("LuaDevelopIcon.ico"));
        ...
        public void OnUpdateMainFormDialogTitle()
        {
            ...
            if (project != null) this.Text = project.Name + " - LuaDevelop";
            else if (document != null && document.IsEditable)
            {
                String file = Path.GetFileName(document.FileName);
                this.Text = file + " - LuaDevelop";
            }
            else this.Text = "LuaDevelop";
        }
        ...
        /// <summary>
        /// Saves all settings of the LuaDevelop
        /// </summary>
        ...
        public void SaveAsTemplate(Object sender, System.EventArgs e)
        {
            ...
            {
                this.saveFileDialog.FileName = "";
                this.saveFileDialog.DefaultExt = ".ldt";
                String prevFilter = this.saveFileDialog.Filter;
                String templateFilter = TextHelper.GetString("Info.TemplateFilter");
                this.saveFileDialog.Filter = templateFilter + "|*.ldt";
                String prevRootPath = this.saveFileDialog.InitialDirectory;
                ...
                if (silentInstall || MessageBox.Show(message, caption, MessageBoxButtons.OKCancel, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    ...
                    while ((entry = zis.GetNextEntry()) != null)
                    {
                        ...
                        if (entry.IsFile)
                        {
                            ...
                            if (File.Exists(fdpath) && (ext == ".dll" || ext == ".ldb" || ext == ".ldl"))
                            {
                                fdpath += ".new";
        ...
        /// <summary>
        /// Checks for updates from LuaDevelop.org
        /// </summary>
        ...
        public void SelectTheme(Object sender, System.EventArgs e)
        {
            ...
            if (ofd.ShowDialog(this) == DialogResult.OK)
            {
                ...
                if (ext == ".ldi") ThemeManager.LoadTheme(ofd.FileName);
                else
                {
        ...
        public void BackupSettings(Object sender, EventArgs e)
        {
            try
            {
                String dirMarker = "\\LuaDevelop\\";
                SaveFileDialog sfd = new SaveFileDialog();
                sfd.AddExtension = true; sfd.DefaultExt = "ldz";
                sfd.Filter = TextHelper.GetString("LuaDevelop.Info.ZipFilter");
